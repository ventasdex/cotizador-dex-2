  ...
  try{
    ...
    target.innerHTML=`
      ...
      <div class="result-card"><h3>Precio DEXI</h3>${p.recommended?`
        ...
        ${p.floor?`<p><b>Piso comercial DEX:</b> ${money(p.floor)} + IVA <small>· por debajo de este monto conviene revisar margen/autorización.</small></p>`:''}${p.requestedParticipants?`<p><b>Tamaño del grupo:</b> ${p.requestedParticipants} participantes · ${p.participantComparableCount?`${p.participantComparableCount} referencia(s) con tamaño comparable conocido.`:'las referencias actuales no documentan participantes; DEXI redujo su peso comercial.'}</p>`:''}
        ...
        <div class="price-evidence-grid">
          <div class="price-evidence-card">
            ...
            ${Array.isArray(p.matrixComparables)&&p.matrixComparables.length?`<ul>${p.matrixComparables.slice(0,4).map(r=>`<li><span class="relevance-badge relevance-${escapeHtml(r.relevance||'contextual')}">${escapeHtml(r.relevanceLabel||'Contextual')}</span> ${escapeHtml(r.course)} <span>${r.hours||'—'} h · ${r.participants?`${r.participants} participantes`:'participantes s/d'} · ${money(r.amount)} · peso ${Math.round((Number(r.weight)||0)*100)}%</span></li>`).join('')}</ul>`:''}
          </div>
          <div class="price-evidence-card">
            ...
            ${Array.isArray(p.historicalComparables)&&p.historicalComparables.length?`<ul>${p.historicalComparables.slice(0,4).map(r=>`<li><span class="relevance-badge relevance-${escapeHtml(r.relevance||'contextual')}">${escapeHtml(r.relevanceLabel||'Contextual')}</span> ${escapeHtml(r.training)}${r.client?` <em>· ${escapeHtml(r.client)}</em>`:''} <span>${r.hours||'—'} h · ${r.participants?`${r.participants} participantes`:'participantes s/d'} · ${money(r.amount)} · peso ${Math.round((Number(r.weight)||0)*100)}%</span>${r.sourceType==='supabase'?` <small>· cierre real</small>`:''}</li>`).join('')}</ul>`:''}
          </div>
        </div>
